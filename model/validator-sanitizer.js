...
function isMobile(location, mobileNumber) {
  const validator = getFunctionName(location);
  return validator(mobileNumber).isMobilePhone().withMessage('Mobile number is not valid please check if you have entered a 10 digit mobile number');
}
 ...
 * @returns validation chain object of express-validator
 */
function isEmailOrMobileOrUsername(location, username) {
  const validator = getFunctionName(location);
  return validator(username)
    .custom((paramUsername) => {
      if (
        validatorLibrary.isEmail(paramUsername) ||
        validatorLibrary.isMobilePhone(paramUsername) ||
        validatorLibrary.isAlphanumeric(paramUsername, 'en-US', { ignore: '.' })
      ) {
        return true;
      }
      throw false;
...
function isValidEmployeeRole(location, role) {
  const validator = getFunctionName(location);
  return validator(role)
    .custom(async (paramRole, { req }) => {
      const beRole = paramRole || null;
      try {
        const [rows] = await pool.execute('SELECT emp_role FROM employee_roles WHERE emp_role = ? AND emp_role_res_id = ?', [beRole, req.user.resId]);
        if (rows.length === 1) return true;
        throw false;
      } catch (e) {
        const beValidateCountrySelectError = error.errList.internalError.ERR_COUNTRY_VALIDATION_SELECT_FAILURE;
...
/**
 * Validates the entered Restaurant_Type to be a valid Restaurant_Type
 *
 * @param location
 * @param Restaurant_Type
 *
 */
function isValidRestaurantType(location, restaurantType) {
  const validator = getFunctionName(location);
  return validator(restaurantType)
    .custom(async (paramRestaurantType) => {
      const beRestaurantType = paramRestaurantType || null;
      try {
        const [rows] = await pool.execute('SELECT restaurant_type FROM restaurant_types WHERE restaurant_type = ?', [beRestaurantType]);
        if (rows.length === 1) return true;
        throw false;
      } catch (e) {
        const beResTypeSelect = error.errList.internalError.ERR_RESTAURANT_TYPE_SELECT_LIST_FAILURE;
        // eslint-disable-next-line no-undef
        return res.status(500).send(responseGenerator.internalError(beResTypeSelect));
      }
    })
 ...
 *
 */
function isValidCurrencyCode(location, cc) {
  const validator = getFunctionName(location);
  return validator(cc)
    .custom(async (paramCC) => {
      const beCC = paramCC || null;
      try {
        const [rows] = await pool.execute('SELECT cc_code FROM currency_codes WHERE cc_code = ?', [beCC]);
        // console.log(rows);
        if (rows.length >= 1) return true;
        throw false;
      } catch (e) {
 ...
 *
 */
function isValidTransactionCategory(location, category) {
  const validator = getFunctionName(location);
  return validator(category)
    .custom(async (paramCategory) => {
      try {
        const [rows] = await pool.execute('SELECT trans_category FROM transaction_categories WHERE trans_category = ?', [paramCategory || null]);
        if (rows.length >= 1) return true;
        throw false;
      } catch (e) {
        const beValidationError = error.errList.internalError.ERR_TRANSACTION_CATEGORY_VALIDATION_SELECT_FAILURE;
        // eslint-disable-next-line no-undef
        return res.status(500).send(responseGenerator.internalError(beValidationError));
      }
    })
    .withMessage('Please choose a valid transaction category.');
}
 ...
 *
 */
function isValidTransactionMode(location, mode) {
  const validator = getFunctionName(location);
  return validator(mode)
    .custom(async (paramMode) => {
      try {
        const [rows] = await pool.execute('SELECT transaction_mode FROM transaction_modes WHERE transaction_mode = ?', [paramMode || null]);
        if (rows.length >= 1) return true;
        throw false;
      } catch (e) {
        const beValidationError = error.errList.internalError.ERR_TRANSACTION_CATEGORY_VALIDATION_SELECT_FAILURE;
    ...
    .withMessage('Please choose a valid transaction mode.');
}
 ...
 *
 */
function isValidTransactionType(location, type) {
  const validator = getFunctionName(location);
  return validator(type)
    .custom(async (paramType) => {
      try {
        if (['CREDIT', 'DEBIT', 'CASHBACK', 'IN-VALID'].includes(paramType || null)) return true;
        throw false;
      } catch (e) {
        const beValidationError = error.errList.internalError.ERR_TRANSACTION_CATEGORY_VALIDATION_SELECT_FAILURE;
        // eslint-disable-next-line no-undef
    ...
    .withMessage('Please choose a valid transaction type.');
}
 ...
 * @returns validation chain object of express-validator
 */
function isEmployeeID(location, fieldName) {
  const message = `Invalid Employee ID. Please provide valid Employee ID'`;
  const validator = getFunctionName(location);
  return validator(fieldName)
    .custom((paramFieldName) => {
      if (hf.isValidID(paramFieldName)) return true;
 ...
 * @returns validation chain object of express-validator
 */
function ifExistIsEmployeeID(location, fieldName) {
  const message = `Invalid Employee ID. Please provide valid Employee ID'`;
  const validator = getFunctionName(location);
  return validator(fieldName)
...
function isUsername(location, username) {
  ...
  const validator = getFunctionName(location);
  // eslint-disable-next-line newline-per-chained-call
  return validator(username).trim().isAlphanumeric('en-US', { ignore: '.' }).withMessage(message).isLength({ min: 3, max: 100 }).withMessage(message);
}
...
const returnValidations = (validationsFor, type) => (req, res, next) => {
  req.logger.verbose(`*** Running validations ${validationsFor} - ${type} ***`);
  const errors = getValidationResult(req);
  if (!errors.isEmpty()) {
    req.logger.warn(`*** Validation Errors ${validationsFor} - ${type} ***`, responseGenerator.validationError(errors));
    return res.status(422).send(responseGenerator.validationError(errors));
  }
  next();
};
...
module.exports.isEmailOrMobileOrUsername = isEmailOrMobileOrUsername;
module.exports.isUrl = isUrl;
module.exports.ifExistsIsUrl = ifExistsIsUrl;
module.exports.returnValidations = returnValidations;
...
module.exports.isValidCurrencyCode = isValidCurrencyCode;
module.exports.isValidCity = isValidCity;
module.exports.isValidTransactionCategory = isValidTransactionCategory;
module.exports.isValidTransactionMode = isValidTransactionMode;
module.exports.isValidTransactionType = isValidTransactionType;
...
module.exports.isEmployeeID = isEmployeeID;
module.exports.ifExistIsEmployeeID = ifExistIsEmployeeID;
...
module.exports.isValidRestaurantType = isValidRestaurantType;
