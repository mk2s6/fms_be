...
/**
 * Check if the given string is UUID or not
 *
 * @param {string} val
 *
 * @returns {boolean} Representing whether the string is UUID or not
 */
function isUUID(val) {
  return validatorLibrary.isUUID(val);
}
...
function isValidID(id) {
  ...
  if (trimmedID !== '' && hasOnlyDigits(trimmedID)) {
    return true;
  }
  return false;
}
